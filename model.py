...
class InitialProjector(nn.Module):
    ...
    def sort_dims(self, embeds):
        abs_sums = embeds.square().sum(0)
        _, indices = t.sort(abs_sums)
        return embeds[:, indices]
    
    def perm_dims(self, embeds):
        perm = t.randperm(embeds.shape[1])
        return embeds[:, perm]
    
    def perm_ids(self, embeds):
